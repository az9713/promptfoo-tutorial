...
These assertion types represent different trade-offs between speed, cost, and evaluation capability. Deterministic assertions (left column) are instantaneous, free, and perfectly reproducible — use them whenever possible for structural checks. LLM-graded assertions (middle column) provide nuanced quality evaluation but require additional API calls, adding latency and cost — use them for subjective criteria like "is this response helpful?" RAG-specific assertions (right column) are specialized for evaluating retrieval-augmented generation systems, measuring whether answers are grounded in the provided context. The Meta assertions (cost, latency, finish-reason) don't evaluate content at all — they verify operational constraints like budget limits and response time requirements.
...
This mapping helps bridge your existing programming knowledge to the JavaScript/TypeScript ecosystem. The most important conceptual shift is from threads to async/await — JavaScript is single-threaded with an event loop, so all I/O operations (network requests, file reads, database queries) are non-blocking and use Promises. The async/await syntax makes this feel synchronous, but under the hood, the event loop is multiplexing many concurrent operations on a single thread. This is why Node.js excels at I/O-heavy applications like web servers and API clients — exactly the kind of work promptfoo does when calling multiple LLM APIs concurrently.
