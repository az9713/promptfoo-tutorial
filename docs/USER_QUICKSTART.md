...
The evaluation matrix is the primary view in the dashboard. Each row represents a test case (a specific input scenario), and each column represents a provider (an LLM model). Each cell shows whether that test case passed or failed for that provider, along with a numeric score between 0 and 1. This matrix view makes it immediately obvious which models perform better on which types of inputs, enabling data-driven model selection. Clicking any cell expands to show the full prompt that was sent, the complete LLM output, and detailed assertion results explaining why each check passed or failed.
