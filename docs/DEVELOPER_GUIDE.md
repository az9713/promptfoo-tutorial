...
This tech stack reflects modern Node.js development conventions. TypeScript provides type safety similar to Java while running on the JavaScript runtime. Express.js is a minimal HTTP framework — think of it as a lightweight Spring Boot. React handles the browser UI as a component tree. Vite is the modern replacement for Webpack, providing fast development builds with hot module replacement. SQLite with Drizzle ORM gives type-safe database access without requiring a separate database server. Biome handles linting (catching code quality issues) and formatting (consistent code style), replacing the older ESLint+Prettier combination for most checks.
...
This shows the full runtime stack from user input to rendered output. When you type `promptfoo eval`, Commander.js parses the command-line arguments and routes to the appropriate command handler. The TypeScript code (compiled to JavaScript by tsdown) runs on the Node.js runtime, which provides the event loop, filesystem access, and HTTP capabilities. External packages from node_modules provide functionality like HTTP clients, template engines, and database drivers. Evaluation results are persisted to SQLite via Drizzle ORM, and the React web UI (built by Vite into static HTML/CSS/JS, served by Express.js) reads those results through REST API calls to display the interactive dashboard.
...
The build process runs three independent tasks concurrently. Type-checking (tsc --noEmit) validates all TypeScript code for type errors without producing output files — it's purely a correctness check. The tsdown bundler compiles TypeScript source files into JavaScript that Node.js can execute, placing output in the dist/ directory. The web UI build (npm run build:app) uses Vite to bundle the React frontend into optimized static assets. These three steps are independent and run in parallel to minimize build time.
...
The root directory contains the project's configuration files and entry points. The package.json is the most important file — it lists all dependencies, defines build/test/run scripts, and configures the project metadata. The tsconfig.json controls TypeScript compiler behavior (strictness level, module format, target JavaScript version). The vitest.config.ts configures the test runner. The biome.jsonc defines linting rules — which code patterns are errors, warnings, or allowed. The .husky/pre-commit hook ensures that lint and format checks run automatically before every commit, preventing code quality issues from reaching the repository.
...
These two patterns — Strategy (for providers) and Command (for assertions) — are the backbone of promptfoo's extensibility. The Provider pattern means you can add support for any new LLM API by implementing a single interface, without modifying the evaluator. The Assertion pattern means you can add new grading logic by adding a case to the dispatcher, without modifying how results are collected or displayed. Both patterns achieve the same goal: decoupling the "what" (which provider, which assertion type) from the "how" (the evaluation loop, the grading pipeline).
